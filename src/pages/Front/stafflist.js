...
const StaffList = (props) => {
  ...
  //       let filterList = res?.data?.result.filter(x => x.DepartmentId == props?.location?.state?.data?.DepartmentId)
  ...
  const fetchAlt = () => {
    ...
    .then(res => {
      ...
      console.log(response, "response")
        ...
        const filterStaff = response.filter(x => x.category == props?.location?.state?.data?.DepartmentId)
        console.log(filterStaff, "fil")
        setStaffArr(filterStaff)
  ...
  useEffect(() => {
    ...
    setDeptId(props?.location?.state?.data?.DepartmentId);
    console.log(props?.location?.state.data, "sdhjgkl,d");
        ...
        >
          <div className="container-fluid">
            <h2 className="monte" style={{ fontSize: "30px" }}>
                {props?.location?.state?.data?.DepartmentName}
            </h2>
            <br />
              ...
              {staffArr &&
                staffArr.map((x, i) => {
                  return (
                    <p>
                      {" "}
                      <Link to={{ pathname: "/staffpage", state: { data: x, dept: props?.location?.state?.data?.DepartmentName } }}>
                        {x.staffname}
                      </Link>
                    </p>
                  );
