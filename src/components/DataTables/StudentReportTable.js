...
export default class StudentReportTable extends Component {
	state = {
	...
	exportPDF = () => {
		...
		if(typeof window !== "undefined"){
			...
			const ass = this.props.studentList[0].department + " Department";
			...
			const data = this.props.studentList.map(d => [
				d.sNo,
				d.name,
				d.email,
				// d.department,
			...
			doc.text("STUDENT REPORT", 60, 150);
			...
			doc.save(today + " StudentReport.pdf")
		}
	};
		...
		return (
			<>
				<div>
					...
					<div className="mt-3">
						<button className="btn btn-sm btn-primary" onClick={() => this.exportPDF()}><Unicons.UilFileExport/> Export Report PDF</button>
					</div>
				</div>
			</>
		)
